...
# Anomaly
print("\nLoading anomaly CSV")
# anomalies = pd.read_csv("anomalies_lat_lon_7d.csv")
anomalies = pd.read_csv("anomalies_0801_0804.csv")
print("Finished loading anomaly CSV")

print("\nUpdating anomaly timestamp")
anomalies["timestamp"] = pd.to_datetime(anomalies["timestamp"]).dt.tz_localize(None)
...
for i in range(map_df.first_valid_index(), map_df.first_valid_index()+len(map_df)):
    ...
    anomaly_index = next(iter(anomalies[anomalies['timestamp'] == ts].index), None)
    if (anomaly_index == None):
        continue

    if (anomalies.at[anomaly_index, 'value']):
        # print(i)
...
# anomaly_trace, = coast_ax.plot([], [], '-', lw=1, ms=2)
# anomaly_trace.set_color("red")
...
def animate(i):
    ...
    if (collisions[index]):
        poly_x, poly_y = polygons[index].exterior.xy
        poly.set_data(poly_x, poly_y)
        dotp.set_color('red')
    
    trace.set_data(history_x, history_y)
    time_text.set_text('Time: ' + str(times[index]))
    return trace, time_text, dotp, poly
